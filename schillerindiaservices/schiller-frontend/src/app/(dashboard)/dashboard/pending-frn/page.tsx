...
import { Download, Loader2, Puzzle, Search, X } from "lucide-react";
...
function repairColumnContent(id: number | undefined, repairStatus: string | null | undefined) {
  ...
  if (rs === "RP") {
    return <span className="text-slate-500 text-xs font-medium">RP</span>;
  }
  if (rs === "RC") {
    return <span className="text-slate-500 text-xs font-medium">RC</span>;
  }
  if (id == null) {
...
export default function PendingFrnPage() {
  ...
  const loadPendingFrn = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await ServiceService.getPendingFrn(
        page,
        pageSize,
        scRefDebounced,
        searchDebounced
  ...
  const handleExport = async () => {
    ...
    try {
      const blob = await ServiceService.exportPendingFrnExcel({
      ...
      a.href = url;
      a.download = `PendingFRN_${new Date().toISOString().slice(0, 10)}.xlsx`;
      a.click();
  ...
  return (
    <div className="space-y-5 max-w-[1600px]">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Puzzle className="w-8 h-8 text-rose-500" />
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Pending FRN</h1>
          </div>
      ...
      <section
        ...
        role="search"
        aria-label="Filter pending FRN list"
      >
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <label htmlFor="pending-frn-sc-ref" className="block text-sm font-medium text-slate-700">
              Sc RNo
            </label>
          ...
          <div className="space-y-1.5">
            <label htmlFor="pending-frn-frn-gir" className="block text-sm font-medium text-slate-700">
              Frn No / Def Gir / Sc RNo
            </label>
            <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-sm focus-within:ring-2 focus-within:ring-violet-500/20 focus-within:border-violet-300">
              <Search className="w-4 h-4 text-slate-400 shrink-0" aria-hidden />
              <input
                id="pending-frn-frn-gir"
                name="frnDefGir"
                type="search"
                autoComplete="off"
                value={searchInput}
                ...
                className="bg-transparent outline-none w-full min-w-0 text-sm text-slate-800 placeholder:text-slate-400"
                placeholder="Matches FRN, Def GIR, or Sc RNo…"
              />
            </div>
          </div>
        </div>
                ...
                {(
                  ...
                  <th
                    ...
                    className={cn(
                      ...
                      (label === "PDays" || label === "Repair") && "text-right"
                    )}
                  >
                    {label}
                  </th>
                ...
                rows.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-1.5 whitespace-nowrap">
                      {row.id != null ? (
                        <Link
                          href={`/dashboard/services/${row.id}/edit?source=pending-frn`}
