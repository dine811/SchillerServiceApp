...
import { ServiceService, type BirAdminRecord } from "@/services/service-service";
...
export default function BirAdminPage() {
  const [rows, setRows] = useState<BirAdminRecord[]>([]);
  ...
  const [exportFeedback, setExportFeedback] = useState<{ variant: "error" | "warning"; text: string } | null>(null);
  ...
  const loadList = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await ServiceService.getPendingBirAdmin(page, pageSize, { search: searchDebounced });
  ...
  const handleExport = async () => {
    if (!exportFrom.trim() || !exportTo.trim()) {
      setExportFeedback({
        variant: "warning",
        text: "Select both From and To dates. Export filters pending BIR rows by FQC inward date (fqc_in_date) in that range.",
      });
      return;
    }
    ...
    try {
      const blob = await ServiceService.exportBirExcel({
        status: "Pending",
        from: exportFrom.trim(),
        to: exportTo.trim(),
        division: exportDivision.trim() || "1",
      });
      downloadBlob(blob, `BIR_${new Date().toISOString().slice(0, 10)}.xlsx`);
    } catch (e) {
      setExportFeedback({
        variant: "error",
        text: e instanceof Error ? e.message : "Export failed. Check your connection and try again.",
      });
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    setDeleteId(id);
    try {
      await ServiceService.deleteBir(id);
      await loadList();
    } catch (e) {
    ...
    <div className="space-y-5 max-w-[1800px]">
        ...
        <div>
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mb-1">
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">
              BIR list
              <span className="ml-2 inline-flex items-center rounded-full border border-indigo-200 bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-indigo-900 align-middle">
                Pending
              </span>
            </h1>
          </div>
          <p className="text-slate-400 text-xs mt-1">
            {loading ? (
              ...
              <>
                ...
                {" · "}
                <Link href="/dashboard/bir-closed-admin" className="text-indigo-800 font-medium underline hover:text-indigo-950">
                  Closed BIR
                </Link>
              </>
            )}
      ...
      <section className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="border-b border-slate-100 px-4 py-3 bg-slate-50/50">
          <h2 className="text-sm font-semibold text-slate-800">Export (legacy BIR)</h2>
          <p className="text-xs text-slate-500 mt-0.5">Filters by fqc_in_date, status, optional division.</p>
        </div>
        ...
        <div className="border-b border-slate-100 px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              placeholder="Search division, BIR ref, status…"
              className="w-full pl-9 pr-3 py-2 rounded-xl border border-slate-200 text-sm text-slate-800 bg-slate-50/60 focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400"
            />
          </div>
        </div>
        {hasFilters && (
      ...
      {exportFeedback && (
        <div
          className={cn(
            ...
            exportFeedback.variant === "warning" && "border border-amber-200 bg-amber-50 text-amber-900",
            exportFeedback.variant === "error" && "border border-red-200 bg-red-50 text-red-800"
          )}
          role="status"
        >
          {exportFeedback.text}
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="border-b border-slate-100 bg-slate-50/50 px-4 py-3">
          <h2 className="text-sm font-semibold text-slate-800">Batch Inspection Report</h2>
          <p className="text-xs text-slate-500 mt-0.5">Legacy BIRAdminList.jsp — Pending only.</p>
        </div>
            ...
            <tbody>
              {loading && rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <tr
                    key={row.id}
                    className="group border-b border-slate-50 hover:bg-indigo-50/20 transition-colors last:border-0"
                  >
                    <td className="px-2 py-1.5 text-slate-600 text-xs whitespace-nowrap">{nil(row.fqcInDate)}</td>
                    <td className="px-2 py-1.5 text-slate-700 text-xs max-w-[120px] truncate">{nil(row.division)}</td>
                    <td className="px-2 py-1.5 text-slate-700 text-xs max-w-[100px] truncate">{nil(row.modelName ?? row.model)}</td>
                    <td className="px-2 py-1.5 text-slate-600 text-xs max-w-[100px] truncate">{nil(row.configuration)}</td>
                    <td className="px-2 py-1.5 text-slate-600 text-xs">{nil(row.receivedQty)}</td>
                    <td className="px-2 py-1.5 text-slate-600 text-xs max-w-[80px] truncate">{nil(row.softwrChanges)}</td>
                    <td className="px-2 py-1.5 text-slate-600 text-xs max-w-[80px] truncate">{nil(row.hardwrChanges)}</td>
                    <td className="px-2 py-1.5 text-slate-600 text-xs max-w-[100px] truncate">{nil(row.accesoryDetails)}</td>
                    <td className="px-2 py-1.5 text-slate-600 text-xs max-w-[80px] truncate">{nil(row.cnrRefNo)}</td>
                    <td className="px-2 py-1.5 text-slate-600 text-xs font-mono">{nil(row.birRefNo)}</td>
                    <td className="px-2 py-1.5">
                      <Badge
                        variant="outline"
                        className="!h-5 min-h-0 px-1.5 py-0 text-[10px] leading-none rounded font-normal bg-indigo-50 text-indigo-900 border-indigo-200"
                      >
                        {nil(row.finalStatus)}
                      </Badge>
                    </td>
      ...
      <DeleteRowDialog
        ...
        title="Delete BIR row?"
        description="This removes the row from birmaster. This cannot be undone."
      />
    </div>
  );
}
