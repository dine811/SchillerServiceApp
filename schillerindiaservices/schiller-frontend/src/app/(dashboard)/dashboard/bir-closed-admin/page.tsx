...
export default function BirClosedAdminPage() {
  ...
  /** Inline export feedback (replaces browser alert for validation / API errors). */
  const [exportFeedback, setExportFeedback] = useState<{ text: string; variant: "warning" | "error" } | null>(null);
  ...
  const loadList = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await ServiceService.getCompletedBirAdmin(page, pageSize, {
  ...
  const handleExport = async () => {
    if (!exportFrom.trim() || !exportTo.trim()) {
      setExportFeedback({
        variant: "warning",
        text: "Please select both the From and To dates. The export filters completed BIR records by FQC inward date (fqc_in_date) within that range.",
      });
      return;
    }
    setExportFeedback(null);
    ...
    try {
      ...
      downloadBlob(blob, `BIR_Closed_${new Date().toISOString().slice(0, 10)}.xlsx`);
    } catch (e) {
      setExportFeedback({
        variant: "error",
    ...
    <div className="space-y-5 max-w-[1800px]">
          ...
          <p className="text-slate-400 text-xs mt-1">
            {loading ? (
              ...
              <>
                ...
                {" · "}
                <Link href="/dashboard/bir-admin" className="text-teal-800 font-medium underline hover:text-teal-950">
                  Pending BIR
                </Link>
              </>
            )}
      ...
      <section className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="border-b border-slate-100 px-4 py-3 bg-slate-50/50">
          ...
          <p className="text-xs text-slate-500 mt-0.5">Status Completed, fqc_in_date range (legacy ClosedBIRList_admin.jsp).</p>
        </div>
        ...
        {exportFeedback && (
          <div
            className={
              exportFeedback.variant === "warning"
                ? "mx-4 mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950"
                : "mx-4 mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800"
            }
            role="alert"
            aria-live="polite"
          >
        ...
        <div className="border-b border-slate-100 px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div className="min-w-[140px]">
            ...
            <select
              ...
              className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800"
            >
              <option value="">All (admin / VP / FQC / PS)</option>
              {divisions.map((d) => (
                <option key={d.productId} value={d.divisionName}>
                  {d.divisionName}
                </option>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="border-b border-slate-100 bg-slate-50/50 px-4 py-3">
          <h2 className="text-sm font-semibold text-slate-800">Closed BIR list</h2>
          <p className="text-xs text-slate-500 mt-0.5">final_status = Completed.</p>
        </div>
      ...
      <DeleteRowDialog
        ...
        title="Delete this completed BIR record?"
        description="This permanently removes the selected closed BIR from the list. You cannot restore it after deletion."
      />
    </div>
  );
}
