...
function repairColumnContent(id: number | undefined, repairStatus: string | null | undefined) {
  const rs = (repairStatus ?? "").trim().toUpperCase();
  if (rs === "RP") return <span className="text-slate-500 text-xs font-medium">RP</span>;
  if (rs === "RC") return <span className="text-slate-500 text-xs font-medium">RC</span>;
  if (id == null) return <span className="text-slate-400">—</span>;
  return (
    <Link
      href={`/dashboard/services/${id}/update`}
      className="text-violet-600 font-semibold text-xs hover:underline whitespace-nowrap"
    >
      SR
    </Link>
  );
...
export default function ObPendingPage() {
  ...
  const loadObPending = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await ServiceService.getObPending(
        page,
        pageSize,
        scRefDebounced,
        searchDebounced
  ...
  const handleExport = async () => {
    ...
    try {
      const blob = await ServiceService.exportObPendingExcel({
      ...
      a.href = url;
      a.download = `OBPending_${new Date().toISOString().slice(0, 10)}.xlsx`;
      a.click();
  ...
  return (
    <div className="space-y-5 max-w-[1600px]">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">OB Pending</h1>
          </div>
      ...
      <section
        ...
        role="search"
        aria-label="Filter OB pending list"
      >
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <label htmlFor="ob-pending-sc-ref" className="block text-sm font-medium text-slate-700">
              Sc RNo
            </label>
            <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-sm focus-within:ring-2 focus-within:ring-amber-500/20 focus-within:border-amber-300">
              <Search className="w-4 h-4 text-slate-400 shrink-0" aria-hidden />
              <input
                id="ob-pending-sc-ref"
                name="scRef"
                type="search"
                autoComplete="off"
                value={scRefInput}
          ...
          <div className="space-y-1.5">
            <label htmlFor="ob-pending-frn-gir" className="block text-sm font-medium text-slate-700">
              Frn No / Def Gir No
            </label>
            <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-sm focus-within:ring-2 focus-within:ring-amber-500/20 focus-within:border-amber-300">
              <Search className="w-4 h-4 text-slate-400 shrink-0" aria-hidden />
              <input
                id="ob-pending-frn-gir"
                name="frnDefGir"
                type="search"
                autoComplete="off"
                value={searchInput}
          ...
          <table className="w-full text-xs min-w-[1100px] leading-snug [&_td]:align-middle">
            ...
            <tbody>
              {loading && rows.length === 0 ? (
                ...
                <tr>
                  <td colSpan={17} className="text-center py-14 text-slate-400 text-sm">
                ...
                rows.map((row) => (
                  <tr
                    key={row.id}
                    className="border-b border-slate-50 hover:bg-amber-50/30 transition-colors last:border-0"
                  >
                    ...
                    <td className="px-2 py-1.5 font-mono text-xs font-semibold text-amber-700 whitespace-nowrap">
                      {nil(row.scRefNo)}
                    </td>
                    ...
                    <td className="px-2 py-1.5 whitespace-nowrap">
                      {row.id == null ? (
                        ...
                        <Link
                          href={`/dashboard/services/${row.id}/edit?source=ob-pending`}
                          className="text-violet-600 font-semibold text-xs hover:underline whitespace-nowrap"
                        >
                          Update
                        </Link>
                    ...
                    <td className="px-2 py-1.5 whitespace-nowrap">
                      {repairColumnContent(row.id, row.repairStatus)}
                    </td>
                  </tr>
                ))
              )}
