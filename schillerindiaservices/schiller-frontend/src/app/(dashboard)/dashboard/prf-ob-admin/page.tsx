...
export default function PrfObAdminListPage() {
  ...
  const loadList = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await ServiceService.getPendingPrfobAdmin(page, pageSize, {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `PRF-OB_${new Date().toISOString().slice(0, 10)}.xlsx`;
      a.click();
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mb-1">
            <ClipboardList className="w-8 h-8 text-violet-600 shrink-0" />
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">
              PRF/OB List
              <span className="ml-2 inline-flex items-center rounded-full border border-violet-200 bg-violet-50 px-2 py-0.5 text-xs font-semibold text-violet-800 align-middle">
                Pending
              </span>
            </h1>
          </div>
          <p className="text-slate-400 text-xs mt-1">
            {loading ? (
              ...
              <>
                ...
                {" · "}
                <Link href="/dashboard/prf-ob-closed" className="text-violet-700 font-medium underline hover:text-violet-900">
                  Closed PRF/OB list
                </Link>
              </>
            )}
      ...
      >
        <div className="border-b border-slate-100 bg-slate-50/50 px-4 py-3">
          ...
          <p className="text-xs text-slate-500 mt-1">
            Grid: division + search (type, PRF/OB ref, CRM ref). Export uses <strong>entry date</strong> range and
            division (legacy PRF-OB.xls columns).
          </p>
        </div>
        <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          <div className="space-y-1.5">
            <label htmlFor="prf-ob-division" className="block text-sm font-medium text-slate-700">
              Division (list)
            </label>
            <select
              id="prf-ob-division"
              ...
              className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-300"
            >
          ...
          <div className="space-y-1.5">
            <label htmlFor="prf-ob-search" className="block text-sm font-medium text-slate-700">
              Search
            </label>
          ...
          <div className="space-y-1.5">
            <label htmlFor="prf-ob-export-division" className="block text-sm font-medium text-slate-700">
              Division (export)
            </label>
            <select
              id="prf-ob-export-division"
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="border-b border-slate-100 bg-slate-50/50 px-4 py-3">
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Pending rows only (status_type = Pending). Search matches work type, PRF/OB ref, CRM ref.
          </p>
        </div>
            ...
            <tbody>
              {loading && rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <tr
                    key={row.id}
                    className="group border-b border-slate-50 hover:bg-violet-50/20 transition-colors last:border-0"
                  >
