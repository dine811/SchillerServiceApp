...
export default function ScClosedPage() {
  ...
  const loadScClosed = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await ServiceService.getScClosed(
        page,
        pageSize,
        scRefDebounced,
        searchDebounced
  ...
  const handleExport = async () => {
    ...
    try {
      const blob = await ServiceService.exportScClosedExcel({
      ...
      a.href = url;
      a.download = `SC_CompletedFRN_${new Date().toISOString().slice(0, 10)}.xlsx`;
      a.click();
  ...
  return (
    <div className="space-y-5 max-w-[1600px]">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">SC Completed FRN</h1>
          </div>
      ...
      <section
        ...
        role="search"
        aria-label="Filter SC closed list"
      >
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <label htmlFor="sc-closed-sc-ref" className="block text-sm font-medium text-slate-700">
              Sc RNo
            </label>
          ...
          <div className="space-y-1.5">
            <label htmlFor="sc-closed-frn-gir" className="block text-sm font-medium text-slate-700">
              Frn No / Def Gir No
            </label>
            <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-sm focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-300">
              <Search className="w-4 h-4 text-slate-400 shrink-0" aria-hidden />
              <input
                id="sc-closed-frn-gir"
                name="frnDefGir"
                type="search"
                autoComplete="off"
                value={searchInput}
              ...
              {loading && rows.length === 0 ? (
                ...
                rows.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-1.5 font-mono text-xs font-semibold text-emerald-700 whitespace-nowrap">
                      {nil(row.scRefNo)}
                    </td>
                    ...
                    <td className="px-2 py-1.5 whitespace-nowrap">
                      {row.id == null ? (
                        ...
                        <Link
                          href={`/dashboard/services/${row.id}/edit?source=sc-closed`}
