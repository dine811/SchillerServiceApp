...
import { BadgeCheck, Download, Loader2, Search, X } from "lucide-react";
...
export default function NewClosedPage() {
  ...
  const loadNewClosed = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await ServiceService.getNewClosed(
        page,
        pageSize,
        scRefDebounced,
        searchDebounced
  ...
  const handleExport = async () => {
    ...
    try {
      const blob = await ServiceService.exportNewClosedExcel({
      ...
      a.href = url;
      a.download = `ClosedProduct_${new Date().toISOString().slice(0, 10)}.xlsx`;
      a.click();
  ...
  return (
    <div className="space-y-5 max-w-[1600px]">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <BadgeCheck className="w-8 h-8 text-violet-600" />
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Closed Product</h1>
          </div>
      ...
      <section
        ...
        role="search"
        aria-label="Filter Closed Product list"
      >
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <label htmlFor="new-closed-sc-ref" className="block text-sm font-medium text-slate-700">
              Sc RNo
            </label>
          ...
          <div className="space-y-1.5">
            <label htmlFor="new-closed-frn-gir" className="block text-sm font-medium text-slate-700">
              Frn No / Def Gir No
            </label>
            <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-sm focus-within:ring-2 focus-within:ring-violet-500/20 focus-within:border-violet-300">
              <Search className="w-4 h-4 text-slate-400 shrink-0" aria-hidden />
              <input
                id="new-closed-frn-gir"
                name="frnDefGir"
                type="search"
                autoComplete="off"
                value={searchInput}
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-xs min-w-[1000px] leading-snug [&_td]:align-middle">
            <thead>
                ...
                {(
                  ...
                  <th
                    ...
                    className={cn(
                      ...
                      label === "Days taken to complete" && "text-right"
                    )}
                  >
                    {label}
                  </th>
            ...
            <tbody>
              {loading && rows.length === 0 ? (
                ...
                <tr>
                  <td colSpan={14} className="text-center py-14 text-slate-400 text-sm">
                ...
                rows.map((row) => (
                  <tr
                    key={row.id}
                    className="border-b border-slate-50 hover:bg-violet-50/25 transition-colors last:border-0"
                  >
                    ...
                    <td className="px-2 py-1.5 font-mono text-xs font-semibold text-violet-700 whitespace-nowrap">
                      {nil(row.scRefNo)}
                    </td>
