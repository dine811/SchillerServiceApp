...
public class PRFOBDao {
    ...
    public static Prfobmaster getById(int id) throws ClassNotFoundException, SQLException {
       ...
       while(rs.next())
       {
           ...
           prf.setEntryDate(utilfn.getUIDateFormat(rs.getString("entry_date")));
           prf.setScEngg(rs.getString("sc_engg"));
           prf.setWorkType(rs.getString("work_type"));
          // prf.setRaisedDate(rs.getString("raised_date"));
           prf.setRaisedDate(utilfn.getUIDateFormat(rs.getString("raised_date")));
          // prf.setReceivedDate(rs.getString("received_date"));
           prf.setReceivedDate(utilfn.getUIDateFormat(rs.getString("received_date")));
         ...
         //  prf.setExecutedDate(rs.getString("executed_date"));
           prf.setExecutedDate(utilfn.getUIDateFormat(rs.getString("executed_date")));
    ...
    public  int save(Prfobmaster prf) throws ClassNotFoundException, SQLException
    {
        ...
        // ps.setString(2, prf.getEntryDate());
         ps.setDate(2, utilfn.getDbDateFormat(prf.getEntryDate()));
          ps.setString(3, prf.getScEngg());
           ps.setString(4, prf.getWorkType());
           // ps.setString(5, prf.getRaisedDate());
            ps.setDate(5, utilfn.getDbDateFormat(prf.getRaisedDate()));
            // ps.setString(6, prf.getReceivedDate());
             ps.setDate(6, utilfn.getDbDateFormat(prf.getReceivedDate()));
                     ...
                     ps.setString(14, prf.getPrfobRefNo());
                      ps.setString(15, prf.getCrmRefNo());
                       ...
                       //  ps.setString(18, prf.getExecutedDate());
                         ps.setDate(18, utilfn.getDbDateFormat(prf.getExecutedDate()));
    ...
    public void update(Prfobmaster prf) throws ClassNotFoundException, SQLException
    {
        ...
        con=DbConnection.getConnection();
        UtilFunctions utilfn = new UtilFunctions();
        PreparedStatement ps=con.prepareStatement("update prfobmaster set crm_ref_no=?,"
                + "remarks=?,status_type=?,"
                        + "executed_date=?,work_type=?"
                                + ",received_date=?,raised_date=? ,"
                                        + "warrenty_status=?,prfob_ref_no=?,part_type=?"
                                                + ",part_description=?,sc_engg=?,rec_dt_frm_sc=? where id=?");
      
        
        ps.setString(1, prf.getCrmRefNo());
        ps.setString(2, prf.getRemarks());
        ps.setString(3, prf.getStatusType());
        ps.setDate(4, utilfn.getDbDateFormat(prf.getExecutedDate()));
        ps.setString(5, prf.getWorkType());
        ps.setDate(6, utilfn.getDbDateFormat(prf.getReceivedDate()));
        ps.setDate(7, utilfn.getDbDateFormat(prf.getRaisedDate()));
        ps.setString(8, prf.getWarrentyStatus());
        ps.setString(9, prf.getPrfobRefNo());
        ps.setString(10, prf.getPartType());
        ps.setString(11, prf.getPartDescription());
        ps.setString(12, prf.getScEngg());
        ps.setString(13, prf.getReceive_date_from_sc());  //prf.getId()
        ps.setInt(14, prf.getId());
