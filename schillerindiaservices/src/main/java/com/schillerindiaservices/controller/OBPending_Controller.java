 ...
 * @author ShineLoGics
 */
public class OBPending_Controller extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet OBPending_Controller</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet OBPending_Controller at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
...
try
     {
         ...
         String[] cols = { "sc_ref_no", "repaired_brd_stk_date", "division_name","frn_no","cust_name","product_model","unit_status","def_mod_brd_name","def_gir_no","type_of_work","ship_dt_frm_ser_cntr","final_remarks","disp_adv_no"};
    ...
    if(sArray.size()==1){
        ...
        individualSearch += sArray.get(sArray.size()-1);
         ...
         try
         {
             String sql = "SELECT count(*) FROM service_master sm INNER JOIN dropdownmaster dm ON sm.unit_status=dm.dd_id WHERE dm.ddvalue IN(\"OW\",\"LAMC\") AND (sm.ship_dt_frm_ser_cntr IS NULL )";
...
//                 String sql1 = "SELECT * FROM " + table;
                   String sql1 = "SELECT * FROM  service_master sm INNER JOIN dropdownmaster dm ON sm.unit_status=dm.dd_id WHERE dm.ddvalue IN(\"OW\",\"LAMC\") AND (sm.ship_dt_frm_ser_cntr IS NULL )" ;
...
//            ja.put(product_model);
        ...
        String sql2 = "SELECT count(*) FROM service_master sm INNER JOIN dropdownmaster dm ON sm.unit_status=dm.dd_id WHERE dm.ddvalue IN(\"OW\",\"LAMC\") AND (sm.ship_dt_frm_ser_cntr IS NULL )";
        if (searchTerm != "")
        {
            sql2 = sql2 + searchSQL;
        ...
        //             localThrowable3.addSuppressed(localThrowable2);
            ...
            Logger.getLogger(OBPending_Controller.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
