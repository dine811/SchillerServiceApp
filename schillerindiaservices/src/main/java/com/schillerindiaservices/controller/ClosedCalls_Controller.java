 ...
 * @author ShineLoGics
 */
public class ClosedCalls_Controller extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ClosedCalls_Controller</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ClosedCalls_Controller at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
       ...
       try
     {
         ...
         String[] cols = { "id","division", "entry_date","call_date","sc_engg","engineer","model","type_of_call","remarks","status_type"};
         String table = "Callregister";
        ...
        individualSearch += sArray.get(sArray.size()-1);
         ...
         try
         {
             int nam=0;
               ...
               String divName=CallRegisterDao.getDivName(nam);
             ...
             String sql = "SELECT COUNT(*) FROM callregister_demo WHERE status_type='Completed' ";
             ps = conn.prepareStatement(sql);
             rs = ps.executeQuery();
            
               }else
               {
             String sql = "SELECT COUNT(*) FROM callregister_demo WHERE division='"+divName+"' AND status_type='Completed' ";
             ps = conn.prepareStatement(sql);
             rs = ps.executeQuery();
               }
               if (rs.next()) {
...
//                 String sql1 = "SELECT * FROM " + table;
                if(role.equalsIgnoreCase("admin")||role.equalsIgnoreCase("ViceChancellor"))
                {
                 sql1 = "SELECT * FROM callregister_demo WHERE status_type='Completed' ";
                }else{
                    sql1 = "SELECT * FROM callregister_demo WHERE division='"+divName+"'AND status_type='Completed' ";
                }
                String unit_status="";
                String type_of_work="";
               ...
               String globeSearch = " and (engineer like '%" + searchTerm + "%'" +" or type_of_call like '%" + searchTerm + "%'" +" or status_type like '%" + searchTerm + "%')" + "" ;
        ...
        while (rs1.next())
        {
            ...
            String date = rs1.getString("entry_date");
            System.err.println("date is --->  "+date);
           String strdate = utilfunctions.getDateFormat(date); 
           System.err.println(strdate);
            ja.put(utilfunctions.getDateFormat(date));
            ja.put(utilfunctions.getDateFormat(rs1.getString("call_date")));
            String engg=CallRegisterDao.getEnggName(rs1.getString("sc_engg"));
            ja.put(engg);
            ja.put(rs1.getString("engineer"));
            ja.put(rs1.getString("model"));
            ja.put(rs1.getString("type_of_call"));
...
//            ja.put(diffDays);
        ...
        if(role.equalsIgnoreCase("admin")||role.equalsIgnoreCase("Vicechancellor"))
        {
            sql2 = "SELECT COUNT(*) FROM callregister_demo ";
        }
        else{
            sql2 = "SELECT COUNT(*) FROM callregister_demo WHERE division='"+divName+"' ";
        }
        if (searchTerm != "")
        {
            sql2 = sql2 + searchSQL;
