 ...
 * @author ShineLoGics
 */
@WebServlet("/PRFOBEngg_PageController_closed")
public class PRFOBEngg_PageController_closed extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet PRFOBEngg_PageController</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet PRFOBEngg_PageController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
    	System.out.println("insidee req controller closed prfob");
       try
     {
         PrintWriter out = response.getWriter();
         String[] cols = { "id","division", "entry_date","sc_engg","work_type","received_date","branch","engineer","model","warrenty_status","prfob_ref_no","executed_date","status_type","crm_ref_no"};
    ...
    if(sArray.size()==1){
        ...
        individualSearch += sArray.get(sArray.size()-1);
         ...
         {  
             ...
             String namee=EmployeeDao.getempdivision(nam);
               String divName=PRFOBDao.PrfObDivName(namee);
               System.out.println(divName+"THE DIVV ISSS");
               String sql="";
                  sql = "SELECT COUNT(*) FROM prfobmaster WHERE division='"+divName+"' and status_type='Completed'";
...
//                 String sql1 = "SELECT * FROM " + table;
                 String sql1 = "SELECT * FROM prfobmaster WHERE division='"+divName+"' and status_type='Completed'";
               ...
               String globeSearch = " and (work_type like '%" + searchTerm + "%'" +" or prfob_ref_no like '%" + searchTerm + "%'" +" or crm_ref_no like '%" + searchTerm + "%')" + "" ;
        ...
        while (rs1.next())
        {
           ...
           ja.put(rs1.getString("division"));
            ...
            ja.put(engg);
            ja.put(rs1.getString("work_type"));
            ja.put(rs1.getString("received_date"));
            String branch=BranchDao.getbName(rs1.getString("branch"));
            ja.put(branch);
            String engineer=EmployeeDao.getEmpName(Integer.parseInt(rs1.getString("engineer")));
            ja.put(engineer);
            ja.put(rs1.getString("model"));
            ja.put(rs1.getString("warrenty_status"));
            ja.put(rs1.getString("prfob_ref_no"));
            ja.put(rs1.getString("crm_ref_no"));
            ja.put(rs1.getString("status_type"));
             ja.put(rs1.getString("executed_date"));
...
//            ja.put(diffDays);
        ...
        String sql2 = "SELECT COUNT(*) FROM prfobmaster WHERE division='"+divName+"' and status_type='Completed'";
        if (searchTerm != "")
        {
            sql2 = sql2 + searchSQL;
