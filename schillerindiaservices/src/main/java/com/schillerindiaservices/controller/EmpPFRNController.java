...
import com.schillerindiaservices.Dao.EmpPFRNDao;
 ...
 * @author ShineLoGics
 */
public class EmpPFRNController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EmpPFRNController</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EmpPFRNController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      try
     {
    	  System.out.println("insie empPfrn controller");
    	   
         PrintWriter out = response.getWriter();
         String[] cols = { "sc_ref_no", "ser_id","sc_engnr", "repaired_brd_stk_date", "division_name","frn_no","cust_name","product_model","unit_status","def_mod_brd_name","def_gir_no","final_remarks","type_of_work","ship_dt_frm_ser_cntr","disp_adv_no","repair_status"};
    ...
    if(sArray.size()==1){
        ...
        individualSearch += sArray.get(sArray.size()-1);
         ...
         int total = 0;
         System.out.println("before db con");
         ...
         try
         {
             int nam=0;
               ...
               System.out.println("before query execute  "+divName);
             String sql = "SELECT count(*) FROM service_master sm INNER JOIN dropdownmaster dm ON sm.unit_status=dm.dd_id AND ddvalue NOT IN(\"OW\",\"LAMC\") AND (sm.ship_dt_frm_ser_cntr IS NULL) AND sm.division='"+divName+"'";
             PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery();
            // System.out.println("after query execute  "+rs.getFetchSize());
           //  System.out.println("after query status" + rs.first());
             if (rs.next()) {
             ...
             try
             {
            	 System.out.println("after query execute 2"); 
                 String searchSQL = "";
//                 String sql1 = "SELECT * FROM " + table;
                 String sql1 = "SELECT * FROM service_master sm INNER JOIN dropdownmaster dm ON sm.unit_status=dm.dd_id AND ddvalue NOT IN(\"OW\",\"LAMC\") AND (sm.ship_dt_frm_ser_cntr IS NULL) AND sm.division='"+divName+"'";
        ...
        while (rs1.next())
        {
        	...
        	System.out.println("inside pfrn loop");
        	System.out.println("inside pfrn loop size  " +rs1.getFetchSize());
...
//            ja.put(product_model);
            ...
            //pending days
                    ...
                    System.out.println("date_from_serCenter  "+date_from_serCenter);
        ...
        String sql2 = "SELECT count(*) FROM service_master sm INNER JOIN dropdownmaster dm ON sm.unit_status=dm.dd_id AND ddvalue NOT IN(\"OW\",\"LAMC\") AND (sm.ship_dt_frm_ser_cntr IS NULL) AND sm.division='"+divName+"'";
        if (searchTerm != "")
        {
            sql2 = sql2 + searchSQL;
...
//                   String empdivision=(String)session.getAttribute("division");
      EmpPFRNDao.pendingFRNExcel(request, response);
    }
