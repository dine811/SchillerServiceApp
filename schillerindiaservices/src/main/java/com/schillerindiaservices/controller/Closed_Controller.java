 ...
 * @author ShineLoGics
 */
public class Closed_Controller extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Closed_Controller</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Closed_Controller at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
...
try
     {
         ...
         String[] cols = { "sc_ref_no", "repaired_brd_stk_date", "division_name","frn_no","cust_name","product_model","unit_status","def_mod_brd_name","def_gir_no","type_of_work","ship_dt_frm_ser_cntr","disp_adv_no"};
    ...
    if(sArray.size()==1){
        ...
        individualSearch += sArray.get(sArray.size()-1);
         ...
         try
         {
             String sql = "SELECT count(*) FROM service_master WHERE ship_dt_frm_ser_cntr IS NOT NULL  AND (repaired_brd_stk_date IS NOT NULL ) AND (ship_date_from_commercial IS NULL )";
             ...
             {
                 String searchSQL = "";
                 String sql1 = "SELECT * FROM service_master WHERE ship_dt_frm_ser_cntr IS NOT NULL  AND (repaired_brd_stk_date IS NOT NULL  ) AND (ship_date_from_commercial IS NULL )";
...
//            ja.put(product_model);
        ...
        String sql2 = "SELECT count(*) FROM service_master WHERE ship_dt_frm_ser_cntr IS NOT NULL  AND (repaired_brd_stk_date IS NOT NULL ) AND (ship_date_from_commercial IS NULL )";
        if (searchTerm != "")
        {
            sql2 = sql2 + searchSQL;
        ...
        //             localThrowable3.addSuppressed(localThrowable2);
            ...
            Logger.getLogger(Closed_Controller.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
