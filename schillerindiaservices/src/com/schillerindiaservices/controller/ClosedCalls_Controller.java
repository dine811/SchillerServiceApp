    ...
    if(sArray.size()==1){
        ...
        individualSearch += sArray.get(sArray.size()-1);
         ...
         try
         {
             ...
             String sql = "SELECT COUNT(*) FROM callregister WHERE status_type='Completed' ";
             ps = conn.prepareStatement(sql);
             rs = ps.executeQuery();
            
               }else
               {
             String sql = "SELECT COUNT(*) FROM callregister WHERE division='"+divName+"' AND status_type='Completed' ";
             ps = conn.prepareStatement(sql);
             rs = ps.executeQuery();
               }
               if (rs.next()) {
...
//                 String sql1 = "SELECT * FROM " + table;
                if(role.equalsIgnoreCase("admin")||role.equalsIgnoreCase("ViceChancellor"))
                {
                 sql1 = "SELECT * FROM callregister WHERE status_type='Completed' ";
                }else{
                    sql1 = "SELECT * FROM callregister WHERE division='"+divName+"'AND status_type='Completed' ";
                }
                String unit_status="";
                String type_of_work="";
        ...
        while (rs1.next())
        {
           ...
           ja.put(rs1.getString("division"));
            ...
            ja.put(rs1.getString("call_date"));
...
//            ja.put(diffDays);
        ...
        if(role.equalsIgnoreCase("admin")||role.equalsIgnoreCase("Vicechancellor"))
        {
            sql2 = "SELECT COUNT(*) FROM callregister ";
        }
        else{
            sql2 = "SELECT COUNT(*) FROM callregister WHERE division='"+divName+"' ";
        }
        if (searchTerm != "")
        {
            sql2 = sql2 + searchSQL;
