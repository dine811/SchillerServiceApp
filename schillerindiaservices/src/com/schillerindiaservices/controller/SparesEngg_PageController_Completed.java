 ...
 * @author ShineLoGics
 */
@WebServlet("/SparesEngg_PageController_Completed")
public class SparesEngg_PageController_Completed extends HttpServlet {
    ...
    if(sArray.size()==1){
        ...
        individualSearch += sArray.get(sArray.size()-1);
         ...
         try
         {
             ...
             String sql = "SELECT COUNT(*) FROM sparemaster WHERE division='"+divName+"' AND final_status='Completed'";
...
//                 String sql1 = "SELECT * FROM " + table;
                 String sql1 = "SELECT * FROM sparemaster WHERE division='"+divName+"' AND final_status='Completed'";
        ...
        while (rs1.next())
        {
            ...
            ja.put(rs1.getString("supplier"));
             String model=rs1.getString("model");
            ja.put(model);
            ja.put(rs1.getString("issued_by"));
            ja.put(rs1.getString("def_Mod_Brd_name"));
            ja.put(rs1.getString("reason"));
            ja.put(rs1.getString("reference"));
            ja.put(rs1.getString("gir_no"));
            ja.put(rs1.getString("sc_engg"));
...
//            ja.put(diffDays);
        ...
        String sql2 = "SELECT COUNT(*) FROM sparemaster WHERE division='"+divName+"' AND final_status='Completed'";
        if (searchTerm != "")
        {
            sql2 = sql2 + searchSQL;
