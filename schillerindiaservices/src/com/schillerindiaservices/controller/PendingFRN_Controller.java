 ...
 * @author ShineLoGics
 */
public class PendingFRN_Controller extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet PendingFRN_Controller</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet PendingFRN_Controller at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
...
try
    ...
    if(sArray.size()==1){
        ...
        individualSearch += sArray.get(sArray.size()-1);
         ...
         try
         {
             String sql = "SELECT count(*) FROM service_master sm INNER JOIN dropdownmaster dm ON sm.unit_status=dm.dd_id AND ddvalue NOT IN(\"OW\",\"LAMC\") AND (sm.ship_dt_frm_ser_cntr IS NULL )";
...
//                 String sql1 = "SELECT * FROM " + table;
                 String sql1 = "SELECT * FROM service_master sm INNER JOIN dropdownmaster dm ON sm.unit_status=dm.dd_id AND ddvalue NOT IN(\"OW\",\"LAMC\") AND (sm.ship_dt_frm_ser_cntr IS NULL )";
...
//             ja.put(rs1.getString("product_model"));
        ...
        String sql2 = "SELECT count(*) FROM service_master sm INNER JOIN dropdownmaster dm ON sm.unit_status=dm.dd_id AND ddvalue NOT IN(\"OW\",\"LAMC\") AND (sm.ship_dt_frm_ser_cntr IS NULL )";
        if (searchTerm != "")
        {
            sql2 = sql2 + searchSQL;
        ...
        //             localThrowable3.addSuppressed(localThrowable2);
            ...
            Logger.getLogger(PendingFRN_Controller.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
