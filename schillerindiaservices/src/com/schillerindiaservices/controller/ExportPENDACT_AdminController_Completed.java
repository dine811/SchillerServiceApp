...
import com.schillerindiaservices.Dao.Export_PendingActDao;
 ...
 * @author ShineLoGics
 */
@WebServlet("/ExportPENDACT_AdminController_Completed")
public class ExportPENDACT_AdminController_Completed extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ExportPENDACT_AdminController</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ExportPENDACT_AdminController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
        ...
        individualSearch += sArray.get(sArray.size()-1);
         ...
         try
         {
             int nam=0;
               ...
               {
                 sql = "SELECT COUNT(*) FROM pendingactmaster WHERE  status_type='completed'";  
               }else{
            sql = "SELECT COUNT(*) FROM pendingactmaster WHERE division='"+divName+"' AND status_type='completed'";
               }
             ...
             try
             {
                 ...
                 if(role.equalsIgnoreCase("admin")||role.equalsIgnoreCase("vicechancellor"))
                 {
                     sql1 = "SELECT * FROM pendingactmaster WHERE  status_type='completed'";
                 }else{
                 sql1 = "SELECT * FROM pendingactmaster WHERE division='"+divName+"' AND status_type='completed'";
                 }
                String unit_status="";
                String type_of_work="";
...
//            ja.put(diffDays);
        ...
        if(role.equalsIgnoreCase("admin")||role.equalsIgnoreCase("vicechancellor"))
        {
            sql2 = "SELECT COUNT(*) FROM pendingactmaster WHERE status_type='completed'";
        }else{
        sql2 = "SELECT COUNT(*) FROM pendingactmaster WHERE division='"+divName+"' AND status_type='completed'";
        }
        if (searchTerm != "")
        {
            sql2 = sql2 + searchSQL;
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
    	System.out.println("inside pedning contrler");
      ...
      // String status="completed";
       ...
       String status="Completed";
       Export_PendingActDao.PendingAdmin(from,to,division,status, request, response);
    }
