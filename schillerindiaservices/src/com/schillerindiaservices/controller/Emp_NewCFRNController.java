...
import com.schillerindiaservices.Dao.New_EmpCFRNDao;
 ...
 * @author ShineLoGics
 */
public class Emp_NewCFRNController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Emp_NewCFRNController</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Emp_NewCFRNController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      try
     {
         ...
         String[] cols = {  "sc_ref_no","sc_engnr","frn_no","cust_name","product_model","unit_status","def_mod_brd_name","def_gir_no","type_of_work","ship_dt_frm_ser_cntr","disp_adv_no","ship_date_from_commercial"};
         ...
         try
         {
             ...
             String sql = "SELECT count(*) FROM service_master WHERE ship_dt_frm_ser_cntr IS NOT NULL  \n" +
                            "AND (repaired_brd_stk_date IS NOT NULL )\n" +
                               "AND (ship_date_from_commercial IS NOT NULL ) AND service_master.division='"+divName+"'";
             ...
             try
             {
                 ...
                 String searchSQL = "";
                 String sql1 = "SELECT * FROM service_master WHERE ship_dt_frm_ser_cntr IS NOT NULL  \n" +
                                "AND (repaired_brd_stk_date IS NOT NULL )\n" +
                               "AND (ship_date_from_commercial IS NOT NULL ) AND service_master.division='"+divName+"'";
...
//            ja.put(product_model);
            ...
            ja.put(type_of_work);
             //pending days
                    ...
                    String commr_date=rs1.getString("ship_date_from_commercial");
                    if(commr_date.equalsIgnoreCase(""))
                    {
                         ...
                         DateFormat df = new SimpleDateFormat("dd-MM-yyyy"); 
                             java.util.Date startDate;
                                 startDate = df.parse(startDateString);
                                  ...
                                  java.util.Date currentdate;
//                              String  current_date=df.format(dateobj);
                            currentdate=df.parse(commr_date);
        ...
        String sql2 = "SELECT count(*) FROM service_master WHERE ship_dt_frm_ser_cntr IS NOT NULL  \n" +
                            "AND (repaired_brd_stk_date IS NOT NULL )\n" +
                               "AND (ship_date_from_commercial IS NOT NULL ) AND service_master.division='"+divName+"'";
        if (searchTerm != "")
        {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        try {
            New_EmpCFRNDao.NewCFRNExcel(request, response);
        } catch (ParseException ex) {
            Logger.getLogger(Emp_NewCFRNController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
