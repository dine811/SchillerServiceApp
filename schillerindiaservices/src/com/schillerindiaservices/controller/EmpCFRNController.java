...
import com.schillerindiaservices.Dao.EmpCFRNDao;
 ...
 * @author ShineLoGics
 */
public class EmpCFRNController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EmpCFRNController</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EmpCFRNController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      try
     {
         ...
         String[] cols = {  "sc_ref_no","sc_engnr","frn_no","cust_name","product_model","unit_status","def_mod_brd_name","def_gir_no","type_of_work","ship_dt_frm_ser_cntr","disp_adv_no"};
         ...
         try
         {
            ...
            // dc_no IS NOT NULL and dc_no!='' AND
             String sql = "SELECT count(*) FROM service_master s WHERE ship_dt_frm_ser_cntr IS NOT NULL  AND (repaired_brd_stk_date IS NOT NULL ) AND (ship_date_from_commercial IS NULL ) AND  s.division='"+divName+"'";
             ...
             try
             {
                 ...
                 String searchSQL = "";
                 String sql1 = "SELECT * FROM service_master s WHERE ship_dt_frm_ser_cntr IS NOT NULL  AND (repaired_brd_stk_date IS NOT NULL ) AND (ship_date_from_commercial IS NULL ) AND s.division='"+divName+"'";
                 
                
                 
                 String searchTerm = request.getParameter("search[value]");
//              String globeSearch = " AND (ser_id like '" + searchTerm + "%'" +")" + "" ;
...
//            ja.put(product_model);
            ...
            ja.put(rs1.getString("def_gir_no"));
                     ...
                     System.out.println(type_of_work+"   "+rs1.getString("type_of_work")+"    the typeworkk issas");
            ja.put(type_of_work);
             //pending days
...
ja.put("");
        ...
        String sql2 = "SELECT count(*) FROM service_master s WHERE ship_dt_frm_ser_cntr IS NOT NULL  AND (repaired_brd_stk_date IS NOT NULL ) AND (ship_date_from_commercial IS NULL ) AND s.division='"+divName+"'";
        if (searchTerm != "")
        {
            sql2 = sql2 + searchSQL;
        ...
        //             localThrowable3.addSuppressed(localThrowable2);
            ...
            Logger.getLogger(EmpCFRNController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        try {
            EmpCFRNDao.CFRNExcel(request, response);
